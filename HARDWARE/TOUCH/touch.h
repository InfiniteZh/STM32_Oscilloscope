...
typedef struct
{
	...
	int x[OTT_MAX_TOUCH];  //当前坐标
	int y[OTT_MAX_TOUCH];  //电容屏有最多5组坐标,电阻屏则为x[0],y[0]代表:此次扫描时,触摸屏的坐标用
												 //x[4],y[4]存储第一次按下时的坐标
	u8 sta;  	//笔的状态
						//b7:按下1/松开0; 
						//b6:0,没有按键按下;1,有按键按下. 
...
u8 TP_Read_XY(int *x,int *y);					//双方向读取(X+Y)
u8 TP_Read_XY2(int *x,int *y);					//带加强滤波的双方向坐标读取
